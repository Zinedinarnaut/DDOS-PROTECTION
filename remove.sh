...
/sbin/iptables -t mangle -D PREROUTING -p tcp --tcp-flags SYN,FIN SYN,FIN -j DROP
...
/sbin/iptables -D INPUT -p tcp -m connlimit --connlimit-Dbove 111 -j REJECT --reject-with tcp-reset
...
/sbin/iptables -D port-scanning -p tcp --tcp-flags SYN,ACK,FIN,RST RST -m limit --limit 1/s --limit-burst 2 -j RETURN
/sbin/iptables -D port-scanning -j DROP
/sbin/iptables -D INPUT -p tcp --dport ssh -m conntrack --ctstate NEW -m recent --set
/sbin/iptables -D INPUT -p tcp --dport ssh -m conntrack --ctstate NEW -m recent --update --seconds 60 --hitcount 10 -j DROP

echo "Firewall configuration successfully removed."
